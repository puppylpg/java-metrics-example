 ...
 */
package other.zk.recipes.leader;
 ...
 * </p>
 * <p>
 * 只监听自己的前一个节点。当前一个节点挂了（不管他是不是leader），本节点都重新监听更前的一个节点（或
 * 成为leader如果自己变成第一个了的话）。这样的话当一个node挂了，只有他的后一个节点被通知，调用getChildren。
 * 如果大家都监听leader，当leader挂了，所有人都getChildren，herd effect，zk受不了。
 */
public class LeaderElectionSupport implements Watcher {
    ...
    private void determineElectionStatus() throws KeeperException, InterruptedException {
        ...
        // 该getChildren的操作不许watch，避免herd effect！！！
